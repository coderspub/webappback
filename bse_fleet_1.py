...
app = Flask(__name__)
app.config['JWT_SECRET_KEY'] = 'lightyagami'
jwt = JWTManager(app)
CORS(app)
...
def test():
    if request.method == 'POST':
        print(request.headers['Authorize'])
    ...
    if request.method == 'POST':
        ...
        if result:
            if bcrypt.check_password_hash(result['passwd'],data['passwd']):
                ...
                return jsonify({'status': True, 'reason': 'successful','access_token':access_token,'refresh_token':refresh_token})
            else:
